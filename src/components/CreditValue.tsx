...
const CreditValue = ({ transaction, isCreator }: Props) => {
  ...
  const handleColor = (tr: Transaction) => {
    // console.log(tr);
    if (tr.is_credit) return "text-secondary";
    if (tr.is_debit) return "text-success";
    return "text-warning";
  };
  ...
  return (
    <>
      ...
      <span
        className="badge text-warning text-lowercase"
        id={`hf${currentTransaction?.id}`}
      >
        {formatNumberWithCommas(
          (value || 0) + ((value || 0) * transaction.credit_percent) / 100
        )}
      </span>
